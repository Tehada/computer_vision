...
def MSE(image_1, image_2):
    ...
    for y in range(height):
        for x in range(width):
            value = (int(image_1[y][x]) - int(image_2[y][x])) ** 2
            total += value
...
def Pyramid(red, green, blue):
    ...
    t_red = transform.resize(red, (new_size, red.shape[1] * new_size / red.shape[0]))
    t_green = transform.resize(green, (new_size, green.shape[1] * new_size / green.shape[0]))
    t_blue = transform.resize(blue, (new_size, blue.shape[1] * new_size / blue.shape[0]))
    crops_1 = Pyramid(t_red, t_green, t_blue)
    for channel in crops_1:
        for frame in channel:
            frame *= 2
    t_red = Crop_pixels(red, crops[0][0], crops[0][1], crops[0][2], crops[0][3])
    t_green = Crop_pixels(green, crops[1][0], crops[1][1], crops[1][2], crops[1][3])
    t_blue = Crop_pixels(blue, crops[2][0], crops[2][1], crops[2][2], crops[2][3])
...
def align(bgr_image):
    ...
    colours = Pyramid(channels[0], channels[1], channels[2])
